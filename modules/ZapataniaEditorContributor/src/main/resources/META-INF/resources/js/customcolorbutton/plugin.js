...
/**
 * @fileOverview The "customcolorbutton" plugin that makes it possible to assign
...
( function() {
	...
	CKEDITOR.plugins.add( 'customcolorbutton', {
		requires: 'panelbutton,floatpanel',
		// jscs:disable maximumLineLength
		lang: 'en,es', // %REMOVE_LINE_CORE%
		// jscs:enable maximumLineLength
		icons: '/o/custom-ckeditor-plugin/js/customcolorbutton/icons/bgcolor.png,/o/custom-ckeditor-plugin/js/customcolorbutton/icons/textcolor.png', // %REMOVE_LINE_CORE%
		hidpi: false, // %REMOVE_LINE_CORE%
		init: function( editor ) {
			console.log('isSupportedEnvironment:', CKEDITOR.env ? CKEDITOR.env.isCompatible : 'unknown');
			...
			if ( !CKEDITOR.env.hc ) {
				...
				var contentTransformations,
					normalizeBackground = editor.config.customcolorbutton_normalizeBackground;
				...
				if ( normalizeBackground === undefined || normalizeBackground ) {
					...
					// correctly picked by customcolorbutton plugin.
					contentTransformations = [
						[
							{
								...
								right: function( element ) {
									var style = new CKEDITOR.style( editor.config.customcolorbutton_backStyle, {
			...
			function addButton( options ) {
				var name = options.name,
					...
					style = new CKEDITOR.style( config[ 'customcolorbutton_' + type + 'Style' ] ),
					colorBoxId = CKEDITOR.tools.getNextId() + '_colorBox',
					colorData = { type: type },
					defaultColorStyle = new CKEDITOR.style( config[ 'customcolorbutton_' + type + 'Style' ], { color: 'inherit' } ),
				...
				editor.ui.add( name, CKEDITOR.UI_PANELBUTTON, {
					...
					panel: {
						css: [ CKEDITOR.skin.getPath( 'editor' ) ].concat( config.customcolorbutton_contentsCss ),
					...
					// {@link CKEDITOR.config#customcolorbutton_colors} list as a parameter. If the color could not be found,
					// this method will fall back to the first color from the panel.
					//
					// @since 4.14.0
					// @private
					// @member CKEDITOR.ui.customcolorbutton
					...
					select: function( callback ) {
						var colors = config.customcolorbutton_colors.split( ',' ),
					...
					onOpen: function() {
						...
						if ( config.customcolorbutton_enableAutomatic ) {
				...
				function createColorStyleDefinition() {
					var colorStyleDefinition = editor.config[ 'customcolorbutton_' + type + 'Style' ];
			...
			function renderColors( colorBoxId, clickFn, historyLength ) {
				var output = [],
					colors = config.customcolorbutton_colors.split( ',' ),
					// Tells if we should include "More Colors..." button.
					moreColorsEnabled = editor.plugins.colordialog && config.customcolorbutton_enableMore,
				...
				for ( var i = 0; i < colors.length; i++ ) {
					if ( ( i % editor.config.customcolorbutton_colorsPerRow ) === 0 )
						output.push( '</tr><tr>' );
					...
					output.push( '<a class="cke_colorauto" _cke_focus=1 hidefocus=true',
						...
						' role="option" aria-posinset="1" aria-setsize="', total, '">',
							'<table role="presentation" cellspacing=0 cellpadding=0 width="100%">',
								'<tr>',
									'<td colspan="', editor.config.customcolorbutton_colorsPerRow, '" align="center">',
			...
			*
			* @private
			* @member CKEDITOR.plugins.customcolorbutton
	...
	ColorBox = CKEDITOR.tools.createClass( {
		...
		statics: {
			colorNames: function( editor ) {
				return editor.lang.customcolorbutton.colors;
			},
	...
	ColorHistory = CKEDITOR.tools.createClass( {
		$: function( editor, cssProperty, clickFn ) {
			...
			if ( this.editor.config.customcolorbutton_renderContentColors ) {
		...
		statics: {
			...
			getRowLimit: function( editor ) {
				return editor.config.customcolorbutton_historyRowLimit;
			},

			getCapacity: function( editor ) {
				return ColorHistory.getRowLimit( editor ) * editor.config.customcolorbutton_colorsPerRow;
			},
		...
		_: {
			...
			alignRows: function() {
				for ( var rowIndex = 0; rowIndex < ColorHistory.getRowLimit( this.editor ); rowIndex++ ) {
					if ( this.rows[ rowIndex ].boxes.length <= this.editor.config.customcolorbutton_colorsPerRow ) {
						return;
			...
			countPanelElements: function() {
				var total = this.editor.config.customcolorbutton_colors.split( ',' ).length + this.getLength();

				if ( this.editor.plugins.colordialog && this.editor.config.customcolorbutton_enableMore ) {
					total += 1;
				}

				if ( this.editor.config.customcolorbutton_enableAutomatic ) {
					total += 1;
				}
 ...
 *
 * Read more in the {@glink features/customcolorbutton documentation}
 * and see the {@glink examples/customcolorbutton example}.
 *
 *		config.customcolorbutton_enableMore = false;
 *
 * @cfg {Boolean} [customcolorbutton_enableMore=true]
 * @member CKEDITOR.config
 */
CKEDITOR.config.customcolorbutton_enableMore = true;
 ...
 *
 *		// Brazil colors only.
 *		config.customcolorbutton_colors = '00923E,F8C100,28166F';
 *
 *		config.customcolorbutton_colors = 'FontColor1/FF9900,FontColor2/0066CC,FontColor3/F00';
 *
 *		// CKEditor color palette available before version 4.6.2.
 *		config.customcolorbutton_colors =
 ...
 *
 * @cfg {String} [customcolorbutton_colors]
 * @member CKEDITOR.config
 */
CKEDITOR.config.customcolorbutton_colors = '1ABC9C,2ECC71,3498DB,9B59B6,4E5F70,F1C40F,' +
 ...
 *		config.customcolorbutton_foreStyle = {
 *			element: 'span',
 *			styles: { color: '#(color)' }
 *		};
 *
 ...
 *		};
 *
 * @cfg [customcolorbutton_foreStyle]
 * @member CKEDITOR.config
 */
CKEDITOR.config.customcolorbutton_foreStyle = {
	element: 'span',
	styles: { 'color': '#(color)' },
	overrides: [ {
 ...
 *
 *		// This is actually the default value.
 *		config.customcolorbutton_backStyle = {
 *			element: 'span',
 ...
 *		};
 *
 * @cfg [customcolorbutton_backStyle]
 * @member CKEDITOR.config
 */
CKEDITOR.config.customcolorbutton_backStyle = {
	element: 'span',
	styles: { 'background-color': '#(color)' }
};
 ...
 * and see the {@glink examples/customcolorbutton example}.
 *
 *		config.customcolorbutton_enableAutomatic = false;
 *
 * @cfg {Boolean} [customcolorbutton_enableAutomatic=true]
 * @member CKEDITOR.config
 */
CKEDITOR.config.customcolorbutton_enableAutomatic = true;
 ...
 * and see the {@glink examples/customcolorbutton example}.
 *
 *		config.customcolorbutton_colorsPerRow = 8;
 *
 * @since 4.6.2
 * @cfg {Number} [customcolorbutton_colorsPerRow=6]
 * @member CKEDITOR.config
 */
CKEDITOR.config.customcolorbutton_colorsPerRow = 6;

/**
 * Whether the plugin should convert `background` CSS properties with color only, to a `background-color` property,
 * allowing the [Color Button](https://ckeditor.com/cke4/addon/customcolorbutton) plugin to edit these styles.
 *
 *		config.customcolorbutton_normalizeBackground = false;
 *
 * @since 4.6.1
 * @cfg {Boolean} [customcolorbutton_normalizeBackground=true]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.customcolorbutton_historyRowLimit = 2;
 *
 * @since 4.15.0
 * @cfg {Number} [customcolorbutton_historyRowLimit=1]
 * @member CKEDITOR.config
 */
CKEDITOR.config.customcolorbutton_historyRowLimit = 1;

/**
 * Defines if color history should be initially filled by colors found in content.
 *
 *		config.customcolorbutton_renderContentColors = false;
 *
 * @since 4.15.0
 * @cfg {Number} [customcolorbutton_renderContentColors=true]
 * @member CKEDITOR.config
 */
CKEDITOR.config.customcolorbutton_renderContentColors = true;

/**
 * The CSS file(s) to be used to apply the style to the color button menu content.
 *
 * ```javascript
 * config.customcolorbutton_contentsCss = '/css/myfile.css';
 * config.customcolorbutton_contentsCss = [ '/css/myfile.css', '/css/anotherfile.css' ];
 * ```
 *
 * @since 4.20.2
 * @cfg {String/String[]} [customcolorbutton_contentsCss]
 * @member CKEDITOR.config
 */
CKEDITOR.config.customcolorbutton_contentsCss = [];
